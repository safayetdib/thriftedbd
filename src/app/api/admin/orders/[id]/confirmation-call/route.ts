...
import { confirmationCallSchema } from "@/lib/validations/order.schema";
import { recordConfirmationCall } from "@/lib/services/order.service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = confirmationCallSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const order = await recordConfirmationCall(id, parsed.data, session?.user?.id);
