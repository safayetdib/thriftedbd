  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-ink-900 text-heading-lg">{order.orderNumber}</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-ink-900 text-price text-heading-md">৳{order.total}</span>
        </div>
      </div>
      ...
      {order.riskFlags.length > 0 && (
        <div className="border-sale-500 bg-sale-50 flex flex-wrap gap-2 rounded-none border p-4">
          {order.riskFlags.map((flag: string) => (
            <Badge key={flag} className="text-sale-700 bg-white">
              {flag}
            </Badge>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Section title="Items">
          <ul className="flex flex-col gap-3">
            {order.items.map((item: IOrderItem, i: number) => (
              <li key={i} className="text-body-sm flex items-center justify-between gap-3">
                <div>
                  <p className="text-ink-900 text-body-sm-strong">{item.title.en}</p>
                  <p className="text-mute">Owner: {item.ownerName}</p>
                </div>
          ...
          <div className="border-hairline-soft text-body-sm mt-4 flex justify-between border-t pt-3">
            <span className="text-mute">Shipping fee</span>
            <span className="text-ink-900 text-body-sm-strong">৳{order.shippingFee}</span>
          </div>
        </Section>
        ...
        <Section title="Customer">
          <dl className="text-body-sm flex flex-col gap-2">
            <div className="flex justify-between">
              ...
              <dd className="text-ink-900 text-body-sm-strong">{order.customer.name}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-mute">Phone</dt>
              <dd className="text-ink-900 text-body-sm-strong">{order.customer.phone}</dd>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-ink-900 text-body-sm-strong">{order.payment.method}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-mute">Payment status</dt>
              <dd className="text-ink-900 text-body-sm-strong">{order.payment.status}</dd>
            </div>
          </dl>
        </Section>
        ...
        {order.orderStatus === "PENDING" && (
          <Section title="Confirmation call">
            <p className="text-mute text-body-sm mb-3">
              Current:{" "}
              <span className="text-ink-900 text-body-sm-strong">
                {order.confirmationCall.status}
              </span>{" "}
          ...
          <Section title="Fulfillment">
            ...
            {order.courier.provider && (
              <p className="text-charcoal text-body-sm mb-3">
                Courier: <span className="text-body-sm-strong">{order.courier.provider}</span>
        ...
        <Section title="Status history">
          <ol className="text-body-sm flex flex-col gap-2">
            {order.statusHistory.map((entry: IStatusHistoryEntry, i: number) => (
              <li
                key={i}
                className="border-hairline-soft flex justify-between border-b pb-2 last:border-0"
              >
                <span className="text-ink-900 text-body-sm-strong">{entry.status}</span>
                <span className="text-mute">{new Date(entry.changedAt).toLocaleString()}</span>
              </li>
            ))}
          </ol>
        </Section>
